...
static void on_entry_activate(GtkEntry *entry, gpointer user_data) {
    ...
    if (g_str_has_prefix(text, "http://") || g_str_has_prefix(text, "https://") || g_str_has_prefix(text, "file://")) {
        ...
        const gchar *template =
            g_strcmp0(engine, "Copilot") == 0 ? "https://copilot.microsoft.com/?q=%s" :
...
int main(int argc, char *argv[]) {
	...
	gtk_combo_box_text_append_text(search_menu, "Copilot");
	...
	gtk_combo_box_set_active(GTK_COMBO_BOX(search_menu), 0); // Copilot como predeterminado
